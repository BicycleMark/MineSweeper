...
public class ObservableCollectionJsonConverter : JsonConverter<ObservableCollection<SweeperItem>>
{
    ...
    public override ObservableCollection<SweeperItem> Read(ref Utf8JsonReader reader, Type typeToConvert,
    ...
    /// <param name="options">The serializer options</param>
    public override void Write(Utf8JsonWriter writer, ObservableCollection<SweeperItem> value,
        JsonSerializerOptions options)
    {
        writer.WriteStartArray();

        var itemConverter = new SweeperItemJsonConverter();
        foreach (var item in value) itemConverter.Write(writer, item, options);
