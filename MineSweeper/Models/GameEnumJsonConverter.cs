...
public class GameStatusJsonConverter : JsonConverter<GameEnums.GameStatus>
{
    ...
    public override GameEnums.GameStatus Read(ref Utf8JsonReader reader, Type typeToConvert,
        JsonSerializerOptions options)
    {
        if (reader.TokenType == JsonTokenType.String)
        {
            var enumString = reader.GetString();
            if (enumString == null) return GameEnums.GameStatus.NotStarted; // default value if string is null
            if (Enum.TryParse<GameEnums.GameStatus>(enumString, out var result)) return result;
        }
        else if (reader.TokenType == JsonTokenType.Number)
        {
            var enumValue = reader.GetInt32();
            if (Enum.IsDefined(typeof(GameEnums.GameStatus), enumValue)) return (GameEnums.GameStatus) enumValue;
        }
...
public class GameDifficultyJsonConverter : JsonConverter<GameEnums.GameDifficulty>
{
    ...
    public override GameEnums.GameDifficulty Read(ref Utf8JsonReader reader, Type typeToConvert,
        JsonSerializerOptions options)
    {
        if (reader.TokenType == JsonTokenType.String)
        {
            var enumString = reader.GetString();
            if (enumString == null) return GameEnums.GameDifficulty.Easy; // default value if string is null
            if (Enum.TryParse<GameEnums.GameDifficulty>(enumString, out var result)) return result;
        }
        else if (reader.TokenType == JsonTokenType.Number)
        {
            var enumValue = reader.GetInt32();
